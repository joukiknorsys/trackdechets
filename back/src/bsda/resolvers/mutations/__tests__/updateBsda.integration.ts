...
`,
  ...
  it("can NOT update destinationReceptionWeight without specifying destinationReceptionRefusedWeight", async () => {
    // Given
    const { company, user } = await userWithCompanyFactory(UserRole.ADMIN);
    const bsda = await bsdaFactory({
      opt: {
  ...
  describe("TRA-16750 - Code D9 becomes D9F", () => {
    it("should allow user to update destinationOperationCode with code D9F", async () => {
      // Given
      const { company, user } = await userWithCompanyFactory(UserRole.ADMIN);
      const bsda = await bsdaFactory({
        opt: {
